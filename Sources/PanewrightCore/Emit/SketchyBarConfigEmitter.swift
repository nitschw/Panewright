...
public enum SketchyBarConfigEmitter {
    ...
    public static func emit(_ config: PanewrightConfig) throws -> Files {
        ...
        let workspacesPlugin = """
            ...
            # Show this pill only when its workspace lives on this monitor;
            # highlight it when it's the one currently visible there.
            ...
            [ -z "$MON" ] && MON="$1"
            ON_MON=$("$A" list-workspaces --monitor "$MON" 2>/dev/null | tr -d ' ')
            VISIBLE=$("$A" list-workspaces --monitor "$MON" --visible 2>/dev/null | tr -d ' ')
            if printf '%s\\n' "$ON_MON" | grep -qx "$2"; then
              if [ "$2" = "$VISIBLE" ]; then

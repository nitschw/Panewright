...
public enum SketchyBarConfigEmitter {
    public struct Files: Equatable, Sendable {
        ...
        public var batteryPlugin: String
        public var wifiPlugin: String
    }
    ...
    public static func emit(_ config: PanewrightConfig) throws -> Files {
        ...
        let rc = """
            ...
            $BAR --add item battery right \\
              --set battery update_freq=60 script="$PLUGINS/panewright_battery.sh"

            $BAR --add item wifi right \\
              --set wifi update_freq=30 script="$PLUGINS/panewright_wifi.sh"
        ...
        let batteryPlugin = """
            ...
            if [ -z "$PCT" ]; then
              sketchybar --set "$NAME" label=""  # desktop Mac: no battery
        ...
        let wifiPlugin = """
            #!/bin/bash
            # Generated by Panewright. CLI SSID queries are redacted without
            # Location permission on modern macOS; system_profiler still knows.
            ...
            if [ -n "$SSID" ] && [ "$SSID" != "<redacted>" ]; then
              ...
              sketchybar --set "$NAME" label="wi-fi ✓"
            else
              sketchybar --set "$NAME" label="⚠ no wi-fi"
            fi
            """

...
public enum SketchyBarConfigEmitter {
    ...
    public static func emit(_ config: PanewrightConfig) throws -> Files {
        ...
        let rc = """
            ...
            # when its workspace lives on that display's monitor; the plugin
            # toggles visibility and highlight. Displays beyond the actual
            # count never render.
        ...
        let workspacesPlugin = """
            ...
            # i3-style dynamic pills: draw a workspace only when it lives on this
            # monitor AND is either occupied (has windows) or the one currently
            # visible there. Empty, unfocused workspaces vanish; moving a window
            # onto one makes its pill appear.
            ...
            if [ "$2" = "$VISIBLE" ]; then
              "$BAR" --set "$NAME" drawing=on background.drawing=on \\
                background.color=\(accent) label.color=0xffffffff
            elif printf '%s\\n' "$OCCUPIED" | grep -qx "$2"; then
              "$BAR" --set "$NAME" drawing=on background.drawing=off label.color=\(palette.dim)
            else

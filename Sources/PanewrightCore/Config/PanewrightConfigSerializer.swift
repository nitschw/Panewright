...
public enum PanewrightConfigSerializer {
    public static func emit(_ config: PanewrightConfig) -> String {
        ...
        // Always written, not just under `modifier = "leader"`. A custom
        // leader key that only survives while leader style is selected is a
        // trap: switch the mod key to try something else, switch back, and
        ...
        lines.append("# Correct overlapping windows caused by app minimum sizes.")
        lines.append("enabled = \(config.fitting.enabled)")
